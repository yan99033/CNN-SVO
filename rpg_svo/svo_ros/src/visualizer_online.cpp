...
Visualizer() :
    ...
    point_marker_scale_(vk::getParam<double>("svo/point_marker_scale", 0.005)),
    camera_marker_scale_(vk::getParam<double>("svo/camera_marker_scale", 0.3)),
    frame_marker_scale_(vk::getParam<double>("svo/frame_marker_scale", 0.15))
{
...
void Visualizer::scalarToColormap(double val, cv::Scalar& col)
{
  // Convert val to range [0,1]
  if (val < 10.0)
    val = 10.0;
  else if (val > 50.0)
    val = 50.0;
  ...
  double x_d = floor(a);	//this is the integer part
  double y_d = floor(255 * (a - x_d)); //fractional part from 0 to 255
  int x = (int) x_d;
  int y = (int) y_d;
  switch(x)
  {
      case 0: col = cv::Scalar(0, y, 255);
              break;
      case 1: col = cv::Scalar(0, 255, 255-y);
              break;
      case 2: col = cv::Scalar(y, 255, 0);
              break;
      case 3: col = cv::Scalar(255, 255-y, 0);
              break;
      case 4: col = cv::Scalar(255, 0, 0);
              break;
  }
}
...
//Overloaded function (to publish depth)
void Visualizer::publishMinimal(
    const cv::Mat& img,
    const cv::Mat& depth,
    const FramePtr& frame,
    ...
    const double& timestamp)
{
  ...
  if(img_pub_nth_ > 0 && trace_id_%img_pub_nth_ == 0 && pub_images_.getNumSubscribers() > 0)
  {
    ...
    if(img_pub_level_ == 0)
    {
      for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
      {
        // Compute depth of the feature
        double d = 50.0; // Initialize with max depth
        cv::Scalar depth_col;
        if ((*it)->point) // if not nullptr
        {
          d = (frame->T_f_w_ * (*it)->point->pos_)(2); // pose is a Vector3d
          // Get color from the colormap
          scalarToColormap(d, depth_col);
        }
        else
          continue;
          // depth_col = cv::Scalar(255,255,255);
    ...
    // Save the frames
    std::string img_save_dir("/home/bryan/results/CNN_VO/frame_kitti_00/");
    if (frame->is_keyframe_)
      cv::imwrite(img_save_dir + std::to_string(trace_id_) + "(kf)" + ".jpg", img_rgb);
    else
      cv::imwrite(img_save_dir + std::to_string(trace_id_) + ".jpg", img_rgb);

    // Get the size (assuming same size for depth and image)
    cv::Size sz = img.size();

    // convert to 3 channels and set depth range [0, 50] for visualization
    ...
    // cv::Mat depth_rgb(sz, CV_8UC3);
    // cv::cvtColor(depthmap_visual, depth_rgb, CV_GRAY2RGB);

    // vertical stack the image and depth
    int height = int(sz.height * 2);
    int width = sz.width;
    cv::Mat full(height, width, CV_8UC3);
    cv::Mat top(full, cv::Rect(0, 0, sz.width, sz.height));
    img_rgb.copyTo(top);
    cv::Mat bottom(full, cv::Rect(0, sz.height, sz.width, sz.height));
    ...
    const Map& map)
{
  ...
  if(pub_frames_.getNumSubscribers() > 0 || pub_points_.getNumSubscribers() > 0)
  {
    vk::output_helper::publishCameraMarker(
        ...
        1, camera_marker_scale_, Vector3d(0.,0.,1.));
    vk::output_helper::publishPointMarker(
        pub_points_, T_world_from_vision_*frame->pos(), "trajectory",
        ros::Time::now(), trace_id_, 0, point_marker_scale_, Vector3d(0.,0.,0.5));
...
void Visualizer::removeDeletedPts(const Map& map)
{
  if(pub_points_.getNumSubscribers() > 0)
  {
    for(list<Point*>::const_iterator it=map.trash_points_.begin(); it!=map.trash_points_.end(); ++it)
      vk::output_helper::publishPointMarker(pub_points_, Vector3d(), "pts", ros::Time::now(), (*it)->id_, 2, point_marker_scale_, Vector3d());
  }
}
...
void Visualizer::displayKeyframeWithMps(const FramePtr& frame, int ts)
{
  ...
  vk::output_helper::publishFrameMarker(
      ...
      T_world_cam.translation(), "kfs", ros::Time::now(), frame->id_*10, 0, frame_marker_scale_);
  ...
  for(Features::iterator it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
  {
    ...
    vk::output_helper::publishPointMarker(
        ...
        ros::Time::now(), (*it)->point->id_, 0, point_marker_scale_, Vector3d(1.0, 0.0, 1.0),
        publish_points_display_time_);

    // Publish local map points
    // vk::output_helper::publishPointMarker(
    //     pub_points_, T_world_from_vision_*(*it)->point->pos_, "pts",
    //     ros::Time::now(), (*it)->point->id_+1000000, 0, point_marker_scale_*1.2, Vector3d(0.0, 1.0, 0.0),
    //     ros::Duration(2));
