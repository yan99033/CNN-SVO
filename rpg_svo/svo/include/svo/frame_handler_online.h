...
class FrameHandlerOnline : public FrameHandlerBase
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  FrameHandlerOnline(vk::AbstractCamera* cam);
  virtual ~FrameHandlerOnline();
  ...
  bool isTrackingLost() { return tracking_lost_; }
...
protected:
  ...
  FramePtr buffer_frame_;                       //!< Save the last n keyframes for depth filter
  set<FramePtr> core_kfs_df_;                   //!< A set of previous keyframes for depth filter
  ...
  monodepth::MonoDepth* monodepth_;             //!< Monocular depth estimator
  cv::Size input_size_;                         //!< width and height of the image and depth
  ...
  size_t core_n_kfs_df_;                        //!< Number of previous keyframes used for depth filter
  ...
  float disp_to_depth_;                         //!< The coefficient to convert from disparity to depth
  bool tracking_lost_;                          //!< Quit VO if tracking is lost
  bool is_initialized_;                         //!< True if the system is initialized
  std::string cnn_model_;                       //!< Path to the pre-trained monodepth model (.pb file)
